...
/**
 * @remarks Christopher J Cavage; 09-09-2015:
 *    The YKUSH received by YepKit does not have a Product ID of 0x0042 upon
 *    inspection with 'lsusb -v'.  Ours are discovered to have 0xF2F7.  In the
 *    event we buy more, we should consider re-writing this tool to take input
 *    from the command-line.  Also, it doesn't look to handle multiple YKUSH's 
 *    too well.
 */
#define PRODUCT_ID 0xF2F7 /* 0x0042 */
...
static struct libusb_device_handle *devh = NULL;
 ...
 /**********************************************************************************
  * 
  * MULTIBOARD SUPPORT COMPONENT
  * 
  * Multiboard support is provided just for the board with customized software.
  *	Board PID:
  * 		Device A: 0x0A42
  * 		Device B: 0x0B42
  * 		Device C: 0x0C42
...
int commandMultiBoard(int pid, char cmd) {
    ...
    devh = libusb_open_device_with_vid_pid(NULL, VENDOR_ID, pid);
    if (devh == NULL) {
        fprintf(stderr, "Could not open YKUSH control device\n");
        libusb_close(devh);
        libusb_exit(NULL);
